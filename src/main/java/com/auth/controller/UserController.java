...
//    }
    ...
    public ResponseEntity<?> assignRolesToUser(@PathVariable("id") @Valid @Min(value=1,message="User Id is required") Long id,
    ...
    @GetMapping("/{id}/roles")
    public ResponseEntity<?> getUserRoles(@PathVariable("id") Long id) throws AuthServiceException {
        return ResponseEntity.ok(userService.getUserRoles(id));
    }


    @PostMapping("/{id}/permissions")
    public ResponseEntity<?> getAllowedPermissions(@PathVariable("id") Long id,
